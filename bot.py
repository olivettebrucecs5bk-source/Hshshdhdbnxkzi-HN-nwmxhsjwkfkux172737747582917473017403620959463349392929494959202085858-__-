"""
Roblox Presence Tracker Bot — Optimized for Render
====================================================
Env vars required:
...
CHECK_TIME  = 10      # giây giữa mỗi lần poll  (phải > COOLDOWN)
COOLDOWN    = 5       # giây tối thiểu giữa 2 webhook cùng 1 user
SAVE_EVERY  = 30      # giây giữa các lần auto-save
DATA_FILE   = "data.json"
...
# ─────────────────────────────────────────
#  SESSION  (single, no cookie needed)
# ─────────────────────────────────────────
            ...
            for p in data.get("userPresences", []):
                ...
                with state_lock:
                    ...
                    if (is_switch or is_change) and not in_cooldown:
                        ...
                        # Cập nhật state dù đang cooldown — không bỏ sót
                        sdata["state"]    = state
                        sdata["universe"] = universe

            # Gửi webhook NGOÀI lock
            for ev in events:
...
if __name__ == "__main__":
    if not WEBHOOK_URL:
        log.warning("⚠️  WEBHOOK_URL chưa set!")

    threading.Thread(target=run_asyncio, daemon=True, name="AsyncioLoop").start()
