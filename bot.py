    ...
    for attempt in range(3):
        try:
            async with s.post(WEBHOOK_URL, json={"embeds": [embed]}) as r:
                # Đọc text trước để tránh lỗi mimetype khi Cloudflare chặn HTML
                ...
                if r.status == 429:
                    try:
                        ...
                        retry = 5.0 # Mặc định chờ 5s nếu nhận về trang HTML lỗi
                ...
                if r.status not in (200, 204):
                    # In ra một phần body để debug nếu lỗi không phải 429
                    log.error(f"Webhook failed: {r.status} | body={body_text[:150]}")
                else:
                    with status_lock:
                        bot_status["webhooks_sent"] += 1
                    log.info(f"✅ Webhook → uid={uid} state={state} switch={switch}")
                
                # Nếu không bị 429 thì thoát loop luôn (kể cả lỗi khác hoặc thành công)
                break
        except Exception as e:
